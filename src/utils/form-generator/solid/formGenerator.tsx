...
const FormGenerator = ({fields}: {
    ...
    const [fieldsState, setFields] = createSignal<IFields[]>(fields);
    ...
    return (
        <form class="flex flex-col gap-4 text-lg" onSubmit={(e)=> {e.preventDefault()}}>
            <For each={fieldsState()}>{(field, index) => (
                <fieldset class="flex flex-col text-black">
                    <label for={field.name} class="dark:text-white">{field.label}</label>
                    {
                        (field.type === "select" && field.options) ? <Select field={field} /> :
                            ((field.type=== "password") ? ( <Password  field={field} />):
            ...
            <button type="submit" class="btn bg-tertiary p-3">Submit</button>
        </form>
    )
}


const Select = ({ field }:{field:IFields}) => {
    return (
...
const Password = ({ field }: { field: IFields }) => {
